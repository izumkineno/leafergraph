...
也就是说，旧的主包 execution shim 已不存在；图级执行真源现在是 `@leafergraph/core/execution` 的 `LeaferGraphGraphExecutionHost`。
...
sequenceDiagram
    ...
    participant GraphExec as "@leafergraph/core/execution"
...
这条入口的图级状态机位于 `@leafergraph/core/execution`：
...
`system/timer` 的注册和 tick 现在也归 `@leafergraph/core/execution` 管理：
...
- 不改正式 document
- 不改正式连线路径
- 只更新 overlay 粒子属性并调用 `requestRender()`
