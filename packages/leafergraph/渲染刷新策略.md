...
- `play()` 和 `step()` 先收集全部 `system/on-play` 节点，创建图级 `activeRun`
- 图级宿主维护的是：
  - `runId`
  - `queue`
  - `stepCount`
...
1. 图级宿主先创建 `activeRun`，收集全部 `system/on-play` 节点。
2. 状态切到 `stepping`。
...
如果主包返回失败或要求 fallback，editor 才会退回：

- `graph.replaceGraphDocument(document)`

这条链代表 editor remote authority 模式下的“增量优先”策略。
