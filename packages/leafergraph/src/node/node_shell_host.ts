...
interface LeaferGraphNodeShellHostOptions {
...
> {
  ...
  private createMissingNodeShell(
    ...
    const selectedRing = new Rect({
      ...
      cornerRadius: this.options.style.nodeRadius + this.options.style.selectedRingOutset,
      opacity: 0,
      selectedStyle: {
        stroke: selectedStroke,
        opacity: 0.92
  ...
  private resolveNodeCategory(node: TNodeState): string {
    ...
    return this.options.nodeRegistry.getNode(node.type)?.category ?? this.startCase(node.type);
  }
