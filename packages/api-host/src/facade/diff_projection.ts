 ...
 * @returns 匹配到的图元结果。
 */
function findGraphicById(host: LeaferGraphLayerLike, id: string): unknown {
  const findHost = host;
  return (
    findHost.findId?.(id) ??
    findHost.findOne?.({ id }) ??
...
function findGraphicByIdFallback(
  host: LeaferGraphLayerLike | null | undefined,
  id: string
): unknown {
  if (!host) {
    return undefined;
  ...
  for (const child of host.children ?? []) {
    const match = findGraphicByIdFallback(child as LeaferGraphLayerLike, id);
    if (match) {
      return match;
    }
  }
