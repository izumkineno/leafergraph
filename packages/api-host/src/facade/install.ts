...
export function installLeaferGraphFacade(LeaferGraphClass: {
  ...
  const prototype = LeaferGraphClass.prototype as LeaferGraphInstallablePrototype;

  if (prototype[leaferGraphFacadeInstalledMarker]) {
    return;
  }

  // 先按职责分组把 façade 方法全部挂到原型上，保持 `leafer_graph.ts` 为薄入口。
  defineLeaferGraphFacadeMethods(prototype, leaferGraphRegistryFacadeMethods);
  defineLeaferGraphFacadeMethods(prototype, leaferGraphViewFacadeMethods);
  defineLeaferGraphFacadeMethods(prototype, leaferGraphSelectionFacadeMethods);
  defineLeaferGraphFacadeMethods(prototype, leaferGraphQueryFacadeMethods);
  defineLeaferGraphFacadeMethods(prototype, leaferGraphExecutionFacadeMethods);
  defineLeaferGraphFacadeMethods(prototype, leaferGraphSubscriptionFacadeMethods);
  defineLeaferGraphFacadeMethods(prototype, leaferGraphDocumentFacadeMethods);
  defineLeaferGraphFacadeMethods(prototype, leaferGraphConnectionFacadeMethods);
  defineLeaferGraphFacadeMethods(prototype, leaferGraphMutationFacadeMethods);

  // 最后写入一次性安装标记，避免测试环境或重复导入时重复覆盖原型。
  Object.defineProperty(prototype, leaferGraphFacadeInstalledMarker, {
    configurable: false,
    enumerable: false,
    writable: false,
    value: true
