...
- `@leafergraph/config/graph`
  - `graph.view`、`graph.runtime`、`graph.history`
- `@leafergraph/config/widget`
  - Widget editing 配置与 `resolveWidgetEditingOptions(...)`
- `@leafergraph/config/context-menu`
  - 菜单行为配置
- `@leafergraph/config/leafer`
  - Leafer 与官方插件配置桥接
...
| `@leafergraph/undo-redo` | 消费 `graph.history`，但不会因为写了 config 自动启用 |
| `@leafergraph/context-menu` | 消费菜单配置 |
