...
它通过显式 `host` 适配对象，把复制、粘贴、删除、运行、历史和快捷键文案接到 `@leafergraph/context-menu`，但不反向依赖 `leafergraph` 主包。

## 包定位

适合直接依赖它的场景：

- 你已经在使用 `@leafergraph/context-menu`
- 你希望把节点图常见菜单动作一次性接上
...
| 包 | 关系 |
| --- | --- |
| `@leafergraph/context-menu` | 菜单 runtime |
| `@leafergraph/context-menu-builtins` | 节点图 builtins 集成层 |
| `@leafergraph/contracts` | 创建节点、创建连线、选区更新等共享类型真源 |
| `@leafergraph/node` | 节点快照和连线模型真源 |
| `@leafergraph/shortcuts` | 可选提供快捷键文案，但不会反向依赖 builtins |
| `@leafergraph/undo-redo` | 可选提供 history host，但不会并进这个包 |
