...
- 当前状态：现状优先，保留中长期方向
- 最近校对：2026-03-23
- 适用范围：`packages/node`、`packages/leafergraph`、`examples/editor`、`openrpc/`
...
flowchart LR
    ...
    editor["examples/editor<br/>工作区、命令、authority、bundle"]
...
- `examples/editor` 负责 viewport 壳层、命令、bundle loader、authority session 和 workspace UI
...
| `examples/editor` | Preact 壳层、viewport、菜单、命令、authority session、bundle loader |
...
- OpenRPC 真源：`openrpc/`
- editor 生成器：`examples/editor/tools/generate_from_openrpc.ts`
- 生成目录：`examples/editor/src/session/authority_openrpc/_generated/`
- runtime/adapter：`examples/editor/src/session/authority_openrpc/runtime.ts`
...
- `examples/editor/src/ui/viewport/Connected.tsx`
- `examples/editor/src/ui/viewport/View.tsx`

右键菜单当前真实入口是：

- 主包基础设施：`packages/leafergraph/src/interaction/context_menu.ts`
- editor 菜单绑定：`examples/editor/src/menu/context_menu_bindings.ts`
- editor 菜单解析：`examples/editor/src/menu/context_menu_resolver.ts`
...
- `examples/editor` 提供编辑器壳层、bundle 装载和 authority 会话
