...
`packages/editor/src/loader/types.ts` 当前定义了三类 manifest：
...
- `packages/editor/src/session/websocket_remote_authority_transport.ts`
- `packages/editor/src/session/message_port_remote_authority_transport.ts`
- `packages/editor/src/session/node_process_remote_authority_client.ts`
...
| 层级 | 包 | 核心文件 | 职责 |
| :--- | :--- | :--- | :--- |
| editor 装载入口 | `packages/editor` | `src/loader/runtime.ts` | 读取本地 / 远端 bundle，执行 script，接收 `LeaferGraphEditorBundleBridge.registerBundle(...)`，归一化为 editor 可消费的 manifest |
...
```mermaid
flowchart LR
    script["script bundle"] --> loader["packages/editor/src/loader/runtime.ts"]
...
当前入口主要集中在 `packages/editor/src/loader/runtime.ts`：
