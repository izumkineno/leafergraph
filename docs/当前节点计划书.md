...
- [ ] 将 `onConnectionsChange`、`onExecute` 与运行反馈接入真实调度
  当前 `onConnectionsChange` 已补齐恢复路径补发与宿主级异常隔离，下一步聚焦最小 `onExecute` 入口、输入输出传播与运行反馈

### 10.3 下一步优先事项

- [ ] 先按 `B-4 最小自由连线起步计划` 推进
  固定顺序为：端口几何协议 -> 连接预览 -> `link.create / link.remove / link.reconnect` -> 合法性校验
- [ ] 先补端口 hit test、候选端口高亮和拖拽预览线，不改正式连线序列化结构
- [ ] 在 editor 命令总线中补正式连线命令，并让右键菜单/未来快捷键可复用
- [ ] 在 editor 命令总线补齐连线命令后的 UI 入口
  当前已补 `link.create / link.remove / link.reconnect` 命令、历史记录负载、链接右键菜单删除入口和右键重连会话；下一步把未来快捷键与更完整的合法性反馈统一接到这条命令链
- [ ] 最小自由连线跑通后，再回到 `D-4 执行闭环起步计划`
  固定顺序切回：最小执行宿主 -> 输入 / 输出传播 -> editor 执行入口 -> `D-3` 基础反馈
