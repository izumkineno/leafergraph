...
1. `@leafergraph/node` 已经承担定义、注册、实例化、配置、序列化和模块作用域协议
2. `leafergraph` 主包已经承担唯一 Widget 注册表、宿主渲染、插件上下文和 Widget 编辑宿主
3. `editor` 现在负责工作分页、本地 bundle loader、`GraphViewport` 接线和最小命令控制器
...
flowchart LR
    ...
    subgraph editor_layer["editor / Preact 壳层"]
        editor_app["App 工作分页 与 渲染开关"]
        editor_loader["loader runtime / 文件选择 + script 注入 + 依赖分析"]
        editor_bridge["LeaferGraphRuntime 与 BundleBridge"]
        editor_view["GraphViewport / 菜单 + 选区 + 快捷键"]
        editor_cmd["node_commands + canvas_commands"]
    end

    subgraph host_layer["leafergraph / 宿主层"]
        host_graph["LeaferGraph / createLeaferGraph()"]
        host_plugin["PluginContext / installModule + registerWidget"]
        host_widget_registry["LeaferGraphWidgetRegistry"]
        host_widget_edit["WidgetEditingManager"]
        host_render["节点壳 + 连线 + Widget 渲染宿主"]
        host_menu["Context Menu Manager"]
    end
    ...
    subgraph sdk_layer["@leafergraph/node / SDK 层"]
        ...
        sdk_scope["scope / installNodeModule()"]
        sdk_factory["createNodeState configureNode serializeNode"]
    end

    future_exec["执行闭环 / onConnectionsChange + onExecute + 历史记录"]
    ...
    editor_view --> host_graph
    editor_view --> host_menu

    editor_bridge --> tpl_demo
    editor_bridge --> tpl_node
    editor_bridge --> tpl_widget

    host_graph --> host_plugin
    host_graph --> host_widget_registry
    host_graph --> host_widget_edit
    host_graph --> host_render
    host_graph --> host_menu

    host_plugin --> sdk_scope
    host_plugin --> host_widget_registry
    sdk_scope --> sdk_definition
    sdk_definition --> sdk_registry
    host_widget_registry --> sdk_registry
    sdk_registry --> sdk_factory
    sdk_factory --> host_graph

    host_render --> future_exec
    host_widget_edit --> future_exec
```
