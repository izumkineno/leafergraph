...
| 生命周期基础钩子 | 部分完成 | `onCreate`、`onConfigure`、`onSerialize`、`onConnectionsChange` 已接线；`onExecute` 仍未调度 |
...
- 把当前“类型已定义但还没跑通”的执行链按最小闭环顺序真正落地
- 让下一步工作不再在“宿主拆层 / 命令总线 / 执行系统”之间来回切换，而是先把执行链打通

执行顺序：

1. `D-1` 尾项收口
   - 恢复已有图时补发 `onConnectionsChange(...)`
   - 给 `onConnectionsChange(...)` 增加宿主级异常隔离，避免“图状态已变更但 API 报错”的半完成状态
   - 明确当前 `connected` 语义是“该槽位现在是否仍有连接”，并决定是否保留
2. 建立最小执行宿主
   - 在主包新增明确的执行入口，例如 `executeNode(nodeId, context?)`
   - 让执行入口经过正式运行时装配，而不是散落在 editor 或 demo 中
   - 保持第一版只支持“单节点执行”，不提前引入完整调度器
3. 接通输入 / 输出传播
   - 让节点 `setOutputData(...)` 的结果可以沿现有 `graphLinks` 写回下游节点 `inputValues`
   - 明确槽位映射、未连接输入和多连接输入的第一版行为
   - 保证传播逻辑只依赖正式图状态，不依赖 editor 壳层
4. 定义最小执行顺序
   - 先固定“当前节点执行 -> 输出传播 -> 按连线命中下游节点”的最小策略
   - 第一版先按无环图和单次触发路径实现，不在这一轮解决循环图调度
   - 若执行链需要继续向下游扩散，宿主应有明确的递归或队列策略
5. 给 editor 接入执行入口
   - 在命令总线中补 `node.execute` 或同等级别命令
   - 至少提供一个可见入口，例如节点菜单或调试按钮
   - 提供一个最小外部样例节点，能证明 `onExecute(...)` 已经真实触发
6. 回接 `D-3` 基础反馈
   - 先补最小运行反馈，例如运行中 / 成功 / 失败态或调试文本
   - 节点执行报错时要能在 editor 中被观察到，而不是只停留在控制台

完成标准：

- 恢复图、创建连线、删除连线三条路径都能稳定驱动 `onConnectionsChange(...)`
- 主包至少有一条正式 `onExecute(...)` 调度链路
- 至少一个外部节点样例可以读取输入、写出输出并传播到下游
- editor 中至少有一个明确入口可以触发执行，并能看见最小反馈
...
- `D-4` 当前剩余重点已从“最小反馈可见”切到“执行状态如何进入更完整的执行检查面板、按链路分组的时间线或命令体系”
...
- [ ] 将 `onConnectionsChange`、`onExecute` 与运行反馈接入真实调度
  当前 `onConnectionsChange` 已补齐恢复路径补发与宿主级异常隔离；`onExecute` 已接入主包 `executeNode(...)`、输入 / 输出传播、节点执行事件订阅口、节点检查快照读取口、editor `node.execute` 命令、模板执行样例，以及节点信号灯 / 节点级错误文案 / 右侧信息面板 / 真实执行链时间线 / 最近失败列表 / 菜单描述级别的最小运行反馈，下一步聚焦更完整的执行检查面板与调试承接

### 10.3 下一步优先事项

- [ ] 继续收尾 `D-4 执行闭环起步计划`
  固定顺序为：现有右侧信息面板继续扩成执行检查面板 -> 命令时间线
- [ ] 让现有执行链时间线继续进入更完整的检查面板
  当前已有节点信号灯、节点级错误文案、右侧信息面板、按链路分组的执行检查视图、最近一次执行详情、真实执行链时间线和最近失败列表；下一步优先考虑失败节点聚合视图或命令时间线
- [ ] 让执行状态进入后续调试面板或命令体系
  当前已有主包 `getNodeExecutionState(...)`、节点执行事件订阅口和节点检查快照读取口；下一步可以评估是否需要 editor 侧执行检查面板、命令时间线或失败节点聚合视图
- [ ] 在执行闭环稳定后，再继续推进更细的宿主拆层与命令系统收敛
  保持顺序为：运行反馈收稳 -> 宿主拆层继续 -> 命令总线继续统一 -> 更完整执行系统
...
- 但最小自由连线、命令总线、执行闭环和更细的宿主拆层还没有完成

因此，接下来最重要的主线应该明确为：

**最小自由连线起步计划（端口几何协议 -> 连接预览 -> 正式连线命令 -> 合法性校验） -> 执行闭环起步计划（最小 `onExecute` 入口 -> 输入 / 输出传播 -> editor 执行入口 -> 基础反馈） -> 主包宿主继续拆层 -> editor 命令总线继续收敛 -> 生态与分发收敛**
