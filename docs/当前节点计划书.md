...
flowchart LR
    ...
    subgraph sdk_layer["@leafergraph/node / SDK 层"]
        ...
        sdk_registry["注册表 / NodeRegistry + WidgetDefinitionReader"]
    ...
    subgraph host_layer["leafergraph / 宿主层"]
        ...
        host_plugin["插件上下文 / sdk + ui + registerNode + registerWidget"]
...
当前 `packages/editor/src/app/GraphViewport.tsx` 已经完成下面这些最小接线：
...
2. 外部插件可以拿到 `sdk + ui + registerWidget`
...
- [X] 建立 `NodeRegistry`、主包 `LeaferGraphWidgetRegistry`、模块安装与作用域解析能力
